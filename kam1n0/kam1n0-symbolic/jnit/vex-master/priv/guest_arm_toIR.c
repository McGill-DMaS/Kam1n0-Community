            ...
            )
{
   ...
   stmt( IRStmt_Exit(
			...
			// JVEX FIX
			//IRConst_U32(toUInt((guest_R15_curr_instr_notENC + 2))),
            OFFB_R15T
       ));
}
   ...
   stmt(
      IRStmt_Exit(
         ...
         IRConst_U32(toUInt(guest_R15_curr_instr_notENC | 1)),
		  // jvex modified:
		 //IRConst_U32(toUInt(guest_R15_curr_instr_notENC)),
         OFFB_R15T
      )
   );
}
...
#  define INSN_COND          SLICE_UInt(insn, 31, 28)
   ...
   //vex_printf("HERE \t(arm) 0x%x  \n", (UInt)guest_R15_curr_instr_notENC);

   //vex_printf("HERE! \t(arm) 0x%x 0x%x  \n", (UInt)(guest_R15_curr_instr_notENC), (UInt)(guest_R15_curr_instr_notENC & 3));
...
DisResult disInstr_THUMB_WRK (
             ...
             void*        callback_opaque,
             const UChar* guest_instr,
			 const UChar* guest_instr_org, //jvex added
			 Long guest_instr_delta, //jvex added
             const VexArchInfo* archinfo,
             const VexAbiInfo*  abiinfo,
             Bool         sigill_diag
          )
   ...
   */
   {
	  ...
	  // jvex added:
	  // LOOK BACK INS TO SEE IF THERE IS AN `IT` INSTRUCTION. IF NOT WE SET guaranteedUnconditional as false.
	  
	  
	  //for (i = 0; i <= guest_instr_delta; ++i)
	//	printf("%x ", guest_instr_org[i]);
	  //printf(": %x \n", insn0);
	  Int i;
	  Long n_delta;
	  for(i = -2; i >= -15; i--){
		  n_delta = guest_instr_delta + i;
		  if(n_delta < 0)
			  break;
		  UShort hwp_i = getUShort(&guest_instr_org[n_delta]);
		  //printf("%x -> ",hwp_i);
		  if((hwp_i & 0xFF00) == 0xBF00 && (hwp_i & 0xF) != 0){
  ...
  decode_success:
   ...
   switch (dres.whatNext) {
      case Dis_Continue:
		 ...
		 // jvex modified: llPutIReg(15, mkU32(dres.len + (guest_R15_curr_instr_notENC)));
         break;
      case Dis_ResteerU:
      case Dis_ResteerC:
         llPutIReg(15, mkU32(dres.continueAt));
                         ...
                         Bool         sigill_diag_IN )
{
   ...
   //vex_printf("disInstr_ARM! \t(arm) 0x%x 0x%x  \n", (UInt)(guest_IP_ENCODED), (UInt)(guest_IP_ENCODED & 3));
   //vex_printf(isThumb ? "true\n" : "false\n");
   ...
   if (isThumb) {
      dres = disInstr_THUMB_WRK ( resteerOkFn,
                                  ...
                                  //JVEX FIX: &guest_code_IN[delta_ENCODED - 1],
