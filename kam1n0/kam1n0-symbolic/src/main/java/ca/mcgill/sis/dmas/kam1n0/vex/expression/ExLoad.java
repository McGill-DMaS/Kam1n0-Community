...
public class ExLoad extends VexExpression {
	public VexEndnessType endness;
	public VexVariableType type;
	public VexExpression expression;

	public ExLoad(@JsonProperty("endness") VexEndnessType endness, @JsonProperty("type") VexVariableType type,
			@JsonProperty("expression") VexExpression expression) {
		super();
		this.endness = endness;
		this.type = type;
		this.expression = expression;
		this.tag = VexExpressionType.Iex_Load;
	}

	@Override
	public ComputationNode getNode(ComputationGraph graph, long ina) {
		ComputationNode node = graph.memory.readMem(ina, expression.getNode(graph, ina), type, endness, graph);
		return node;
	}
