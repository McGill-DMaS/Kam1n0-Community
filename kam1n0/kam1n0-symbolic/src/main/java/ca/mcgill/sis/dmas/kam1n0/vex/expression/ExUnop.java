...
public class ExUnop extends VexExpression {

	public VexOperation operation;
	public VexExpression exp1;

	public ExUnop(@JsonProperty("operation") VexOperation operation, @JsonProperty("exp1") VexExpression exp1) {
		this.operation = operation;
		this.exp1 = exp1;
		this.tag = VexExpressionType.Iex_Unop;
	}

	@Override
	public ComputationNode getNode(ComputationGraph graph, long ina) {
		ComputationNode node = new ComputationNode(operation);
		graph.addComputationNode(node, exp1.getNode(graph, ina));
		return node;
	}
	...
	public String toStr(VexToStrState state) {
		return operation.toStr() + "(" + exp1.toStr(state) + ")";
	}
