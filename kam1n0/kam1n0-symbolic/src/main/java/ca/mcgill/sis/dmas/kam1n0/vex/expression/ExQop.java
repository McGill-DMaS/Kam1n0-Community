...
public class ExQop extends VexExpression {

	public VexOperation operation;
	public VexExpression exp1;
	public VexExpression exp2;
	public VexExpression exp3;
	public VexExpression exp4;

	public ExQop(@JsonProperty("operation") VexOperation operation, @JsonProperty("exp1") VexExpression exp1,
			@JsonProperty("exp2") VexExpression exp2, @JsonProperty("exp3") VexExpression exp3,
			@JsonProperty("exp4") VexExpression exp4) {
		this.operation = operation;
		this.exp1 = exp1;
		this.exp2 = exp2;
		this.exp3 = exp3;
		this.exp4 = exp4;
		this.tag = VexExpressionType.Iex_Qop;
	}

	@Override
	public ComputationNode getNode(ComputationGraph graph, long ina) {
		ComputationNode node = new ComputationNode(operation);
		graph.addComputationNode(node, exp1.getNode(graph, ina), exp2.getNode(graph, ina), exp3.getNode(graph, ina),
				exp4.getNode(graph, ina));
		return node;
	}
	...
	public void updateTmpOffset(int newOffset) {
		...
		exp3.updateTmpOffset(newOffset);
		exp4.updateTmpOffset(newOffset);

	}

	@Override
	public String toStr(VexToStrState state) {
		return operation.toStr() + "(" + exp1.toStr(state) + "," + exp2.toStr(state) + "," + exp3.toStr(state) + ","
				+ exp4.toStr(state) + ")";
	}
