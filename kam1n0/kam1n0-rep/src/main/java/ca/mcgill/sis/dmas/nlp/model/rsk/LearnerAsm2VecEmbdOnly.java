...
import ca.mcgill.sis.dmas.nlp.model.astyle._1_original.WordEmbedding;
...
public class LearnerAsm2VecEmbdOnly implements Serializable {

	private static final long serialVersionUID = -743890181829256933L;

	private static Logger logger = LoggerFactory.getLogger(LearnerAsm2VecEmbdOnly.class);
	...
	public transient volatile AtomicLong tknCurrent;
	...
	private void gradientDecend(final Iterable<FuncTokenized> funcs, Map<String, NodeWord> funcMap, long numTkns,
		...
		new Pool(parallelism).start(indx -> {
			...
			gen.subIterable().forEach(func -> {
				// update alpha:
				long val = tknCurrent.get();
				if (val - tknLastUpdate > alphaUpdateInterval) {
					alpha = param.optm_initAlpha * (1.0 - 1.0 * val / (numTkns * iterations + 1));
					alpha = alpha < param.optm_initAlpha * 0.00001 ? param.optm_initAlpha * 0.00001 : alpha;
					if (debug)
						p.report(logger, val, alpha);
					if (stage != null) {
						stage.progress = val * 1.0 / p.totalCount;
					}
					tknLastUpdate = val;
				}
	...
	private void iterate(List<List<String>> in_strs, NodeWord docNode, RandL rl, double[] bfIn, double[] bfNeul1e,
		...
		List<List<NodeWord>> ins = in_strs.stream()
				.map(in -> in.stream().map(tkn -> vocab.get(tkn.trim().toLowerCase()))//
						.peek(node -> tknCurrent.incrementAndGet())//
						.filter(notNull)//
	...
	public LearnerAsm2VecEmbdOnly(Asm2VecNewParam param) {
		this.param = param;
	}
}
