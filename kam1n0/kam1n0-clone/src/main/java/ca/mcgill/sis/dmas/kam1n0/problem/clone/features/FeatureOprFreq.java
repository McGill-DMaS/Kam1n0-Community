...
public class FeatureOprFreq extends Features {
	...
	public void update(SparseVector vec, List<List<String>> blk, ImmutableMap<String, Integer> featMap) {
		for (List<String> tokens : blk) {
			for (int i = 1; i < tokens.size(); ++i) {
				Integer index = featMap.get(tokens.get(i));
				if (index != null)
	...
	public static ArrayList<String> constructFeature(AsmLineNormalizer normalizer) {
		// operand types
		return new ArrayList<>(normalizer.res.getAllOperandValues(normalizer.setting.normalizationLevel));
	}
